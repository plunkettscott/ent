...
func Upsert(t *testing.T, client *ent.Client) {
	...
	id := client.User.Create().
		...
		OnConflict(
			sql.ConflictColumns(user.FieldPhone),
			// Ignore changes for all columns except for the ones below.
	...
	client.Item.Create().SetID("A").OnConflict(sql.ConflictColumns(item.FieldID), sql.ResolveWithIgnore()).ExecX(ctx)
}
